//! Shared building blocks for sapphire-agent-cli clients (CLI + desktop GUI).
//!
//! Both the always-on voice satellite (`sapphire-agent-cli`) and the
//! desktop GUI (`sapphire-agent-desktop`) need the same config schema
