...
- **前提**: PR #261 系（ツールプレフィクス分離 `agent_`/`user_`）は未マージ。
  本仕様はツール名に依存しない書き方（「`subagent` ツール」として記述）をして
  いるが、#261 が先に入ればツール名が変わる点だけ実装時に確認する。
...
- ツール名のプレフィクス分離（#261、`agent_`/`user_`）は本変更の範囲外。
  名前が変わる場合、本仕様の `subagent` 文字列参照箇所が追随するだけ。
