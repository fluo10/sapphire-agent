...
### 5. 再開はセッション検索で行う（永続状態を増やさない）

タスク `T` について:

- **継続先** = `room_id == T` かつ `!is_closed` のセッションのうち最新（`last_at` 最大）。
- **クールダウンの起点** = `room_id == T` のセッションのうち最新の `last_at`
  （無ければ `created_at`。閉じたセッションも含めて探す）。
- **ターン数** = そのセッションの user メッセージ数（`load_session` して数える）。
...
loop {
    ...
    // 1サイクル = 1タスク、最大 max_turns ターン
    session = resume_or_create(picked)                     // 決定 5
    loop {
        if !is_idle() { status = idle("busy: ..."); break } // 現在のターンは最後まで走る
        run_llm_turn(session, "[Autonomous: <name>]\n\n<body>")
        turns += 1
        if turns >= picked.max_turns { store.close_session(session); break }
    }
}
```

**ターン途中の強制中断はしない**（Issue コメントの方針どおり。割り込み注入式の中断は
本家でも未熟なので採用しない）。アイドル判定は**ターンとターンの間**でのみ効く。
検知したらセッションは閉じず、開いたまま保留する — 次に静かになったら同じセッションの
続きから再開する。
...
- **`max_turns` 到達**: そのセッションを `close_session` する。ターン数の導出が
  「開いているセッションの user メッセージ数」なので、閉じることで
  **次のサイクルは新しいセッションで始まる** — クールダウンが明けるまで due に
  ならない限りは。
...
- **アイドル検知による割り込み・中断・再開の機構そのもの**。「他セッションが動いたら
  ターンを中断する」は作らない（決定 8）。アイドル→開始はスコープ内。
...
3. 他セッションに動きがある間は**始まらない**。動きを検知したターンの**後**に止まり、
   次に静かになったら**同じセッションの続き**から再開する（新しいセッションを
   作らない）。
