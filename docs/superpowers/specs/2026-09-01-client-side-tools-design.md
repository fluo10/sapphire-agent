...
- **モデルが `client_shell_start` の後始末を忘れる。** セッションごとに保持中の
  ターミナル数に上限を置く（8）。上限に達したら新規作成を断り、
  どのハンドルが残っているかを結果に列挙する。
- **接続が実行中に切れる。** 接続終了時に、そのセッションが保持しているターミナルを
  すべて `terminal/release` する。

後者は「接続終了時に何かを片付ける」処理だが、**PR #188 で入れて撤回した
「空セッションを閉じる」とは性質が違う。** あれはセッションという永続的なものを
接続の都合で閉じており、再接続を塞いだ。ターミナルは接続に属しており、
接続が消えれば出力の読み手がいない。解放しないほうが誤りである。
