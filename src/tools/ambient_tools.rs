        ...
        Self {
            ...
            spec: ToolSpec {
                ...
                description: "Register an auto-enrolled voice in the workspace, optionally \
                              ...
                              retroactively, because they store the speaker id, not the \
                              name. This is a manual escape hatch: it does not check whether \
                              the voice has cleared the automatic promotion thresholds."
                    .into(),
                input_schema: json!({
                    "type": "object",
                    "properties": {
...
mod tests {
    ...
    fn state() -> (tempfile::TempDir, Arc<Mutex<AmbientToolState>>) {
        ...
        let candidates =
            crate::ambient::speaker::candidates::CandidateStore::open(tmp.path().join("c"))
                .unwrap();
